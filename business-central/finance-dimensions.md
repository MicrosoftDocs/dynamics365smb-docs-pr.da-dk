---
title: Arbejde med dimensioner for at spore og analysere data
...
En dimensionsgruppe er en entydig kombination af dimensionsværdier. De gemmes som dimensionsgruppe poster i databasen. Hver dimensionsgruppepost repræsenterer en enkelt dimensionsværdi. Derudover identificeres hver dimensionsgruppe og dimensionsgruppepost i den af et fælles dimensionsgruppe-id.  
...
* **Globale dimensioner** bruges som filtre, f.eks. i rapporter, kørsler og XMLporte. Du kan kun bruge to globale dimensioner, så vælg dimensioner, du vil bruge ofte.
...
### Sådan konfigureres standarddimensioner for kunder, leverandører og andre konti
...
### Sådan oprettes globale dimensioner og genvejsdimensioner
...
#### Sådan ændres globale dimensioner
...
3. Øverst på siden skal du vælge en af følgende to indstillinger for kørsel af batchjobbet.
    ...
    |-|-|
    |**Fortløbende**|(Standard) Hele dimensionsændringen udføres med én transaktion, der tilbagefører alle poster til de dimensioner, som de havde før ændringen.<br /><br />Denne indstilling anbefales, hvis virksomheden har forholdsvis få bogførte poster, så det vil tage den korteste tid at fuldføre batchjobbet. Processen låser flere tabeller og blokerer andre brugere, indtil den er færdig. Bemærk, at med store databaser fuldføres processen muligvis ikke i denne tilstand. Hvis det er tilfældet, skal du bruge indstillingen **Parallel**.|
    |**Parallel**|Dimensions ændringen foregår i flere baggrundssessioner, og operationen deles op i flere transaktioner. Hvis du vil bruge denne indstilling, skal du slå **Parallel behandling** til/fra . <br /><br />Vi anbefaler denne mulighed til store databaser eller virksomheder med mange bogførte poster, fordi den fuldføres på kortest tid. Bemærk, at opdateringsprocessen ikke starter i denne tilstand, hvis der er mere end én aktiv databasesession.|  
...
## Få et overblik over dimensioner, der bruges flere gange
...
### Sådan får du vist globale dimensioner på sider med poster
