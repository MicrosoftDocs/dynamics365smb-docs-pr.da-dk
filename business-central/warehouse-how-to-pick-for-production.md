---
title: 'Plukke eller flytte varer til produktion, samling eller projekter i grundlæggende lageropsætninger'
...
---
# <a name="pick-for-production-assembly-or-projects-in-basic-warehouse-configurations"></a>Plukke til produktion, montage eller projekter i grundlæggende lagerkonfigurationer
...
I en grundlæggende lagerkonfiguration for det udgående flow (pluk) skal du aktivere følgende indstillinger på **siden Lokation kort**  for lokationen:

* Produktion, skal du vælge **Pluk/bevægelse** (lager) i produktionsforbrugslageret **. Feltet Håndtering.** 
* Samling, vælg **Lagerbevægelse** i **Asm. Forbrug Whse. Feltet Håndtering.** 
* Projektstyring skal du vælge **Pluk** (lager) i Projekt **forbrugslager. Feltet Håndtering.** 

Brug følgende dokumenter for interne operationer:

* Lagerpluk
* Bevægelse af lagerbeholdning

## <a name="inventory-picks"></a>Lagerpluk

* Når du registrerer et pluk for en intern operation, f.eks. en produktion eller et projekt, bogføres forbruget af de plukkede komponenter samtidig.
...
For lokationer, der bruger placeringer, kan du udvide flowet til projekter med dokumenter af **typen Lagerbevægelse** .

> [!NOTE]
> [!INCLUDE[prod_short](includes/prod_short.md)] bruger værdien i feltet **Restantal** på projektplanlægningslinjen, når der oprettes pluk (lager). Hvis du vil bruge pluk fra lager til projekter, skal du aktivere til/fra-knappen **Anvend forbrug sammenkæde**  på **kort**  siden Projekt for projekterne. På den måde kan du spore forbruget i forhold til din plan. Hvis du ikke aktiverer til/fra, forbliver restantallet ved **0**, og lagerplukket bliver ikke oprettet. Du kan finde flere oplysninger i [Sådan konfigureres projektforbrugssporing](projects-how-setup-jobs.md?tabs=current-experience#to-set-up-project-usage-tracking).
...
1. I kildedokumentet, som kan være en produktionsordre eller et projekt, skal du vælge handlingen **Opret læg-på-lager/pluk** (lager).  
...
1. På kildedokumentet, som kan være en produktionsordre, en montageordre eller et projekt, skal du vælge handlingen **Opret læg-på-lager/pluk** (lager).  
2. Markér afkrydsningsfeltet **Opret pluk (lager)**.
3. Vælg knappen **OK**.
...
|Projekter | Ændre et projekts status til **Åben**, eller opret projekt med status Åben med det samme.|  
