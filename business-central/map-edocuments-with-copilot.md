...
> [!IMPORTANT]
> - Dette er en produktionsklar forhåndsversionsfunktion til produktion og sandkassemiljøer i alle landets lokaliseringer<!-- with the exception of Canada -->.
> - Produktionsklar forhåndsversioner er omfattet af supplerende vilkår for anvendelse. Flere oplysninger: [Supplerende vilkår for anvendelse af Dynamics 365 Forhåndsversion](https://go.microsoft.com/fwlink/?linkid=2189520)
...
1. Hvis **Købsordre** for denne bestemte leverandør *findes, og der er et indkøbsordrenummer* i den modtagne **E-dokumentfil**, vil dette [!INCLUDE[prod_short](includes/prod_short.md)] **E-dokument** automatisk blive knyttet til den angivne **Købsordre**. **Dokumentstatus** for dette **E-dokument** er **I gang**, og **E-dokumentstatus** på undersiden **Servicestatus** er **Ordre tilknyttet**.  
...
3. Efter et par sekunder vil **E-dokument match ordrelinjer med Copilot** foreslå linjer til matchning med nogle yderligere detaljer: 
    ...
    |--------|-----------------|
    |Automatisk matchet | Angiver antallet af matches, der foreslås automatisk. Dette tal er baseret på en strengsammenligning, og hvis der er 80 % eller flere beskrivelser, der overlapper hinanden, matcher systemet automatisk disse beskrivelser uden at bruge Copilot-funktioner. |
    ...
    4. Alle fuldt foreslåede og matchede linjer er markeret med grøn farve. Hvis der er et problem, f.eks. en anden pris, men inden for det tilladte prisinterval, markeres denne linje med en gul farve. Hvis der er lighed mellem beskrivelsesfelterne, men prisforskellen er større end tilladt, markeres denne linje med en rød farve.
