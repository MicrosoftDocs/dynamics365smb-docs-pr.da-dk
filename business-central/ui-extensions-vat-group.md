...
Medlemmer af momsgruppen og grupperepræsentanten kan bruge **Opsætning af den assisterede opsætning af momsgruppe styring** til at definere deres ansættelse med gruppen og oprette en forbindelse mellem deres [!INCLUDE[d365fin](includes/d365fin_md.md)]-lejere. Gruppemedlemmerne bruger forbindelsen til at sende deres moms tilbage til gruppens repræsentant. Repræsentanten sender moms til skattemyndighederne på koncernens vegne ved hjælp af en enkelt moms Returvare. 
...
2. Dele de tekniske oplysninger, som momsgruppe medlemmerne og repræsentanten skal bruge for at tilslutte deres [!INCLUDE[d365fin](includes/d365fin_md.md)]-leje.

  Momsgrupperepræsentanten har som regel disse oplysninger, f. eks. navnet på den momsgruppe, som medlemmerne af MOMS-gruppen skal indsende moms til.
3. Oprette brugere i momsgruppens repræsentanters [!INCLUDE[d365fin](includes/d365fin_md.md)], at momsgruppemedlemmerne kan bruge til at godkende og oprette forbindelse.
...
5. Du kan konfigurere udvidelsen af momsgruppe styringen i moms gruppens repræsentant [!INCLUDE[d365fin](includes/d365fin_md.md)] vha vejledningen **Opsætning af momsgruppestyring**. 

> [!NOTE]
> Hvis du vil oprette forbindelse til momsgruppens repræsentant, skal gruppemedlemmerne bruge en bruger med adgang til momsgruppens repræsentants [!INCLUDE[d365fin](includes/d365fin_md.md)]. Repræsentanten for momsgruppen skal imidlertid oprette mindst én bruger af sikkerhedsmæssige årsager. Det anbefales dog, at du opretter en bruger for hvert momsgruppemedlem. Sørg for at distribuere legitimationsoplysningerne for disse brugere til momsgruppemedlemmer på en sikker måde.

## <a name="understanding-the-vat-group-management-setup"></a>Forståelse af opsætning af momsgruppestyring

Repræsentanten for momsgruppen udviser et API, som momsgruppe medlemmerne kan bruge til at oprette forbindelse til deres [!INCLUDE[d365fin](includes/d365fin_md.md)]-lejer og sende moms tilbage. Medlemmerne af momsgruppen vil ofte bruge [!INCLUDE[d365fin](includes/d365fin_md.md)] i separate Azure Active Directory-lejer. Derfor skal mere opsætning være nødvendig, hvis der skal oprettes forbindelse mellem momsgruppemedlemmet og repræsentantens [!INCLUDE[d365fin](includes/d365fin_md.md)]. 
...
Godkendelse håndteres af Azure Active Directory, så du skal oprette forbindelse i momsgruppens repræsentant Azure Active Directory for at tillade forbindelser. En Azure Active Directory-appregistrering skal konfigureres med adgang til momsgruppens repræsentant [!INCLUDE[d365fin](includes/d365fin_md.md)]. Dette gælder også, hvis repræsentanten for momsgruppen bruger [!INCLUDE[d365fin](includes/d365fin_md.md)] on-premises. Derudover skal du konfigurere Single Sign-on, hvis den ansvarlige momsgruppe bruger [!INCLUDE[d365fin](includes/d365fin_md.md)] on-premises.
...
Når repræsentanten for momsgruppen bruger [!INCLUDE[d365fin](includes/d365fin_md.md)] online eller on-premises, bruger momsgruppemedlemmerne Azure Active Directory til at godkende, når de sender momsreturvarer til den ansvarlige momsgruppe. Hvis medlemmerne af momsgruppen også bruger [!INCLUDE[d365fin](includes/d365fin_md.md)] online, kan medlemmet af momsgruppen godkende vha. de angivne legitimationsoplysninger og slutpunktoplysningerne. Du kan finde flere oplysninger i afsnittet [Opsætning af momsgruppemedlem](ui-extensions-vat-group.md#vat-group-member-setup).

Medlemmer af momsgruppe, der har [!INCLUDE[d365fin](includes/d365fin_md.md)] on-premises, skal konfigurere en app-registrering i Azure Active Directory for momsgruppemedarbejderens [!INCLUDE[d365fin](includes/d365fin_md.md)]-lejer. App-registreringen gør det muligt for momsgruppens repræsentants [!INCLUDE[d365fin](includes/d365fin_md.md)] online at godkende Gruppemedlemmet. Du kan finde flere oplysninger om registrering af et program i [Hurtig start: Registrere et program på Microsoft-identitetsplatformen](/azure/active-directory/develop/quickstart-register-app).
...
* Tilføj tilladelser i afsnittet **API-tilladelser** [!INCLUDE[d365fin](includes/d365fin_md.md)]. Aktivér uddelegeret adgang til **Financials.ReadWrite.All** og **user_impersonation**.
...
> [!NOTE]
> Før du begynder, skal du kontakte momsgruppesælgeren for at få følgende oplysninger om deres [!INCLUDE[d365fin](includes/d365fin_md.md)]-lejer:
>
> * API URL
...
1. Vælg ikonet ![Elpære, der åbner funktionen Fortæl mig](media/ui-search/search_small.png "Fortæl mig, hvad du vil foretage dig"), angiv **Momsgruppe**, og vælg derefter det relaterede link.
2. Hvis du vil definere virksomhedens momsgruppe rolle, skal du vælge **Medlem** og derefter vælge **Næste**.
...
4. Tilføj **API URL-adressen** fra momsgrupperepræsentanten. Normalt er URL-adressen formateret på følgende måde: `https://api.businesscentral.dynamics.com/v2.0/[TENANT-ID]/[ENVIRONMENTNAME]`. Eksempel `https://api.businesscentral.dynamics.com/v2.0/907869c3-b252-4aca-b9cb-17a15d25477b/UKRepresentative`. 
5. Tilføj [!INCLUDE[d365fin](includes/d365fin_md.md)]-firmanavnet for momsgrupperepræsentanten, f. eks *CRONUS.UK Ltd*.
6. Vælg **Godkendelsestype**, vælg **OAuth2** og vælg derefter **Næste**.
7. Angiv det id, som repræsentanten for momsgruppen har opgivet, i feltet **Klient-id**.
8. Indtast den hemmelighed, som repræsentanten for momsgruppen har stillet til rådighed, i feltet **repræsentant for momsgruppe**.
9. I **feltet OAuth 2,0 Authority Endpoint**-feltet skal du angive *https://login.microsoftonline.com/common/oauth2*.
10. I feltet **OAuth 2,0 Resource URL** skal du angive *https://api.businesscentral.dynamics.com/*.
11. I feltet **OAuth 2,0 Redirect URL** skal du angive *https://businesscentral.dynamics.com/OAuthLanding.htm*. 
12. Når du har angivet de forskellige felter, skal du vælge **Næste** og derefter angive de brugerlegitimationsoplysninger, som repræsentanten for momsgruppen har stillet til rådighed.
13. Vælg den momsrapport konfiguration, som du kan bruge til at rapportere moms til myndigheder i dit land.
...
Medlemmer af momsgruppen bruger standardprocesserne til at oprette momsreturneringer. Den eneste forskel er at vælge rapportversionen **MOMSGRUPPE**, som sender momsen tilbage til momsgruppens repræsentant i stedet for myndighederne. Du kan finde flere oplysninger i [Om rapporten Momsopgørelse](/business-central/finance-how-report-vat#about-the-vat-return-report).
